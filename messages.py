...
/add - добавить ФИО в чёрный список

Нажмите на /help для вызова меню'''
...
msg_to_admin_conv_closed = f'''Некому слать сообщение. Если вам написал новый пользователь, \
'нажмите Начать диалог. 

Админу доступны следующие команды: 

{admin_commands}'''


msg_to_user_conv_closed = 'Для проверки ФИО по базе нажмите на оператор /search'


msg_user_add_in_blocklist = 'Для внесения ФИО в ЧС писать сюда: почта'


msg_enter_user_real_name = 'Введите реальное имя пользователя, которого надо удалить ' \
                          '(как в записной книжке). Или id пользователя'

msg_access_amin_only = 'Функционал доступен только админу'